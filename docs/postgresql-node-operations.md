...
The Node application is the only database client. It uses parameterized `pg` queries, a versioned migration runner, transactions for account/catalog mutations, and a request-local `app.admin_authenticated` authorization setting. Every application table has RLS enabled and forced; public policies are limited to catalog/settings reads and role-request/aggregate inserts.
...
- `/health/ready` checks the database connection.
