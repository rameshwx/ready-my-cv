...
3. Add all `.env.example` values as Coolify environment values/secrets. Generate separate `SESSION_HASH_PEPPER`, `JOB_HANDLE_PEPPER`, and `DATA_ENCRYPTION_KEY`; add SMTP values for production report delivery. Public verification has no third-party credentials or browser configuration.
